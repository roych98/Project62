...
public class HiredMerchant extends PlayerInteractionManager {
    ...
    private int itemId;
    private IPlayerInteractionManager ipm = null;

    public HiredMerchant(MapleCharacter owner, int itemId, String desc) {
        super(owner, itemId % 10, desc, 3);
        this.itemId = itemId;
        this.map = owner.getMap();
        this.ipm = owner.getInteraction();
        this.schedule = TimerManager.getInstance().schedule(new Runnable() {

            @Override
            public void run() {
                ipm.removeAllVisitors(3, 1);
    ...
    public void buy(MapleClient c, int item, short quantity) {
        ...
        if(pItem == null) {
            AutobanManager.getInstance().autoban(c, "Buying a non-existant item from merchant.");
            return;
        }
        if (pItem.getBundles() > 0) {
            ...
            synchronized (items) {
                ...
                System.out.println("Bundles: " + pItem.getBundles());
                System.out.println("\r\n");*/
                newItem.setQuantity((short) (quantity * pItem.getItem().getQuantity()));
                if (c.getPlayer().getMeso() >= pItem.getPrice() * quantity) {
                    if (quantity > 0 && pItem.getBundles() >= quantity && pItem.getBundles() > 0) {
                        ...
                        AutobanManager.getInstance().autoban(c.getPlayer().getClient(), "Attempted to Merchant dupe.");
                   }
                } else {
