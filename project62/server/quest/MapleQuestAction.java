    ...
    public void run(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            case EXP:
                ...
                int exp;
                if(c.getLevel() > 9)
                    exp = (int)Math.floor(ChannelServer.getInstance(c.getClient().getChannel()).getQuestMultiplyer());
                else 
                    exp = 1;
                c.gainExp(MapleDataTool.getInt(data) * exp, true, true);
                break;
            case ITEM:
                ...
                for (MapleData iEntry : data.getChildren()) {
                    ...
                    if (MapleDataTool.getInt(iEntry.getChildByPath("count")) < 0) { // remove items
                        ...
                        if(c.getInventory(iType).isFull()) {
                            c.dropMessage("Please clear your inventory before completing the quest!");
