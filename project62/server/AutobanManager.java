...
public class AutobanManager implements Runnable {
    ...
    public void autoban(MapleClient c, String reason) {
        if(!c.getPlayer().ignoreHack) {
        for (ChannelServer ch : ChannelServer.getAllInstances()) {
            ch.broadcastGMPacket(MaplePacketCreator.serverNotice(6, "[CHEAT ALERT] " + c.getPlayer() + " " + reason));
        }
    ...
    public synchronized void addPoints(MapleClient c, int points, long expiration, String reason) {
        ...
        if (this.points.get(acc) >= AUTOBAN_POINTS) {
            ...
            if (c.getChannelServer().AutoBan()) {
                ...
                try {
                    c.getChannelServer().getWorldInterface().broadcastGMMessage(null, MaplePacketCreator.serverNotice(6, "[Autoban] " + name + " has been banned by the system. (Reason: " + reason + ")").getBytes());
