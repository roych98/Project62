...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public void damage(MapleCharacter from, int damage, boolean updateAttackTime) {
        ...
        } else if(hasFakeHPBar()){
            from.getMap().broadcastMessage(makeFakeBossHPBarPacket(), getPosition());
        } else if (!isBoss()) {
    ...
    private void giveExpToCharacter(MapleCharacter attacker, int exp, boolean highestDamage, int numExpSharers) {
        ...
        if (attacker.getHp() > 0) {
            ...
            if (attacker.getMapId() < 100000000 && attacker.getLevel() < 10) {
                personalExp /= attacker.getClient().getChannelServer().getExpRate();
            }
            if (isTaunted()) {
   ...
   public MapleCharacter killBy(MapleCharacter killer) {
    ...
    public MaplePacket makeFakeBossHPBarPacket() {
        return MaplePacketCreator.showBossHP(9500317, getHp(), getMaxHp(), (byte) 4, (byte) 5);
        //4,1 green/red
        //3,1 yellowish/red
        //2,1 blue/red
        //1,1 red/red
        //5,1 black/red, looks to be reverse
    }
    public boolean hasBossHPBar() {
        return (isBoss() && getTagColor() > 0) || isHT();
    }
    public boolean hasFakeHPBar() {
        return this.getId() == 9400121 || this.getId() == 9400549 || this.getId() == 9400014;
    }
    ...
    public void sendSpawnData(MapleClient client) {
        ...
        if (hasBossHPBar()) {
            ...
            client.getSession().write(makeFakeBossHPBarPacket());
        }
    }
    ...
    public boolean applyStatus(MapleCharacter from, final MonsterStatusEffect status, boolean poison, long duration, boolean venom) {
        ...
        if (poison) {
            ...
            int webDamage = (int) (getMaxHp() / (50.0 - getController().getSkillLevel(SkillFactory.getSkill(4111003))));
    ...
    public void setJustSpawned(boolean t) {
        this.justSpawned = t;
    }
