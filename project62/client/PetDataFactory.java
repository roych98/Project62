...
public class PetDataFactory {
    ...
    public static PetCommand getPetCommand(int petId, int skillId) {
        PetCommand ret = petCommands.get(String.valueOf(petId) + "." + String.valueOf(skillId));
        if (ret != null) {
            return ret;
        }
        synchronized (petCommands) {
            // see if someone else that's also synchronized has loaded the skill by now
            ret = petCommands.get(String.valueOf(petId) + "." + String.valueOf(skillId));
            //ret = mobSkills.get(String.valueOf(skillId) + "." + String.valueOf(level));
            if (ret == null) {
                ...
                petCommands.put(Integer.valueOf(petId) + "." + Integer.valueOf(skillId), ret);
            }
            return ret;
        }
    }
