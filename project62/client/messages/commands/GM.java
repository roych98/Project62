...
public class GM implements Command {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
        ...
        if (splitted[0].equals("!job")) {
            ...
            try {
                ...
                player.dropMessage(asd.toString() + ". This is not a valid Job ID.");
                return;
            }
            player.setJob(job);
           ...
           mc.dropMessage("You are at position " + player.getPosition());    
        } else if (splitted[0].equals("!positionplayer")){
            MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
            mc.dropMessage("That person is at position" + victim.getPosition());     
        ...
        /*} else if (splitted[0].equals("!opennpc")) {
            ...
            try {
                item = Integer.parseInt(splitted[1]);
                    if (ItemConstants.disallowedGMItems(item) && !player.getName().equals("Cipher") && !player.getName().equals("mars") && !player.getName().equals("Vintage")){
                        player.dropMessage("ItemID " + item + " is not allowed to be made.");
                        return;
                    }
        ...
        } else if (splitted[0].equals("!spychat")) {
                    MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
                    if (splitted.length == 3) {
                        if (victim != null) {
                            victim.clearWatcher();
                            mc.dropMessage("You stopped spying on " + victim.getName());
                        } else {
                        ...
                        if (splitted[1].equalsIgnoreCase("clear")) {
                            ...
                            mc.dropMessage("You stopped spying everyone in your channel.");
                            ...
                            mc.dropMessage("You started watching everyone in your channel. Use '!spychat clear' to stop.");
                        } else {
                            if (victim != null) {
                                if (victim.getWatcher() == null) {
                                    victim.setWatcher(player);
                                    mc.dropMessage("You started spying on " + victim.getName());
                                } else {
                                    if (victim.getWatcher().getName() != null) {
                                        mc.dropMessage(victim.getWatcher().getName() + " is already spying on this player. Use !spychat " + victim.getName() + " clear so you can watch this player instead");
                                    } else {
                                        mc.dropMessage("Someone is already spying on this player. Use !spychat " + victim.getName() + " clear so you can watch this player instead");
                                    }
                                }
                            } else {
                                mc.dropMessage("This player isn't in the same channel as you or logged off");
                        ...
                        mc.dropMessage("Syntax: !spychat <ign> / !spychat <ign> stop / !spychat all / !spychat clear");
                    }
       ...
       /* } else if (splitted[0].equals("!gender")) {
            ...
            if (victim != null) {
                ...
                mc.dropMessage("Level: " + victim.getLevel());
                ...
                mc.dropMessage("Party: " + (victim.getParty() != null ? victim.getParty() : "No party joined. ") + " || PartyID is " + victim.getPartyId());
            } else {
        ...
        } else if (splitted[0].equals("!giftdp")) {
            MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
            if (victim != null) {
                int amount;
                try {
                ...
                int type = getOptionalIntArg(splitted, 3, 1);
                victim.modifyofficialDP(amount);
                victim.dropMessage(5, player.getName() + " has gifted you " + amount + " Donation Points.");
                mc.dropMessage("DP recieved.");
            } else {
                mc.dropMessage("Player not found.");
            }
         } else if (splitted[0].equals("!giftep")) {
            MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
            if (victim != null) {
                int amount;
                try {
                ...
                int type = getOptionalIntArg(splitted, 3, 1);
                victim.modifyEventPoints(amount);
                victim.dropMessage(5, "Congratulations! You have won " + amount + " Event Points.");
                mc.dropMessage("Event Points recieved.");
            } else {
                mc.dropMessage("Player not found.");
            }
        */} else if (splitted[0].equals("!maxskills")) {
            ...
            if (victim != null) {
                ...
                victim.dropMessage(5, "Your smega ability is now " + (victim.getCanSmega() ? "on" + MapleCharacter.unmute : "off" + MapleCharacter.mute));
            ...
            if (victim != null) {
                ...
                victim.dropMessage(5, "Your chatting ability is now " + (victim.getCanTalk() ? "on" + MapleCharacter.unmute : "off" + MapleCharacter.mute));
        ...
        /*} else if (splitted[0].equals("!givedisease")) {
           ...
           //player.getMap().broadcastMessage(MaplePacketCreator.getClock(getOptionalIntArg(splitted, 1, 60)));
            player.getMap().displayClock(player, getOptionalIntArg(splitted, 1, 60));
        } else if (splitted[0].equals("!dclock")) {
            player.getMap().broadcastMessage(MaplePacketCreator.destroyClock());
        ...
        /*} else if (splitted[0].equals("!spawn")) {
            ...
            /*if (num > 20 
                    && !player.getName().equalsIgnoreCase("Vintage")
                    && !player.getName().equalsIgnoreCase("Cipher")) {
                mc.dropMessage("Stop spawning shit");
            }
        ...
        /*} else if (splitted[0].equalsIgnoreCase("!event")) {
            if (c.getChannelServer().eventOn == false) {
                ...
                try {
                    c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(6, c.getChannel(), "[Event] The event has started in Channel " + c.getChannel() + "! Please use @joinevent to participate in the event.").getBytes());
                } catch (RemoteException e) {
                ...
                try {
                    c.getChannelServer().getWorldInterface().broadcastMessage(null, MaplePacketCreator.serverNotice(6, c.getChannel(), "[Event] Entrance to the event has been closed.").getBytes());
                } catch (RemoteException e) {
                    c.getChannelServer().reconnectWorld();
                }
            }
            } else if (splitted[0].equals("!oxfalse")) { 
            if (c.getPlayer().getMapId() == 109020001) { 
                ...
                mc.dropMessage("You are not in the OX Quiz map."); 
            } 
        } else if (splitted[0].equals("!oxtrue")) { 
            if (c.getPlayer().getMapId() == 109020001) { 
                for (MapleCharacter mch : c.getPlayer().getMap().getCharacters()) { 
                    if (mch != null) { 
        ...
        } else if (splitted[0].equals("!bombplayer")) {
                    MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
        ...
        } else if(splitted[0].equals("!ignorehack")) {
            MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
            if(victim == null) {
                player.dropMessage("Invalid input.");
                return;
            }
            victim.ignoreHack = !victim.ignoreHack;
            player.dropMessage(victim.getName() + " ignore hack status: " + victim.ignoreHack);
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{
            ...
            new CommandDefinition("dclock", 3),
            ...
            new CommandDefinition("position", 3),
            new CommandDefinition("bomb", 3),
            new CommandDefinition("bombplayer", 3),
            new CommandDefinition("positionplayer", 3),
            new CommandDefinition("giftdp", 3),
            new CommandDefinition("spychat", 3),
            new CommandDefinition("event", 3),
            new CommandDefinition("oxfalse", 3),
            new CommandDefinition("oxtrue", 3),
            new CommandDefinition("giftep", 3),
            new CommandDefinition("bombmap", 3),
            new CommandDefinition("saveall", 3),
            new CommandDefinition("ignorehack", 3)
        };
    }
}