...
public class Admins implements Command {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
        ...
        if (splitted[0].equals("!speakall")) {
            ...
            if (playernear.isAlive() && playernear != player && !playernear.isGM()) {
                ...
                playernear.dropMessage(5, "Someone added a ; after the text wow");
            }
            }
        } else if (splitted[0].equals("!drop")) {
            MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            int itemId = Integer.parseInt(splitted[1]);
                if (ItemConstants.disallowedGMItems(itemId) && !player.getName().equals("Roy") && !player.getName().equals("rebca")){
                    player.dropMessage("ItemID " + itemId + " is not allowed to be made.");
                    return;
                }
        ...
        } else if (splitted[0].equals("!fullmap")) {
            ...
            if (splitted[1].equalsIgnoreCase("seal")) {
                ...
                type = 128;
             } else if (splitted[1].equalsIgnoreCase("switchcontrols")) {
                type = 124;
            } else {
                mc.dropMessage("Hey Nino JR type an actual fucking proper option.");
                return;
            }
           ...
           Collection<MapleCharacter> victims = c.getPlayer().getMap().getCharacters();
            ...
            victim.canTalk(!victim.getCanTalk());
                ...
                player.dropMessage(6, "The maps chatting ability is now set to " + victim.getCanTalk());
            } else {
        ...
        } else if (splitted[0].equals("!rdrops")) {
        ...
        } else if (splitted[0].equalsIgnoreCase("!rmap")) {
            int mapId = Integer.parseInt(splitted[1]);
            MapleMap map = c.getChannelServer().getMapFactory().getMap(mapId);
            boolean force = splitted.length >= 3 && splitted[2].equals("force");
            if (force) {
        ...
        } else if (splitted[0].equals("!hottime")) {
            final ChannelServer che = cserv;
            che.setExpRate((int)Math.floor(che.getExpRate() * 1.5));
            che.setDropRate(che.getDropRate() + 1);
            final Collection<ChannelServer> cservs = ChannelServer.getAllInstances();
            final int ch = c.getChannel();
            for (ChannelServer ce : cservs) {
                ce.broadcastPacket(MaplePacketCreator.serverNotice(6, "[Hot Time Event] has started for the next 30 minutes in channel " + ch + "! More information on our forums!"));
                ce.setServerMessage("Hot Time Event is now live in channel " + ch + "! More information on our forums!");
            }
                che.hottimeTimer = TimerManager.getInstance().schedule(new Runnable() {
                    @Override
                    public void run() {
                        che.setExpRate((int)Math.floor(che.getExpRate() / 1.5));
                        che.setDropRate(che.getDropRate() - 1);
                        for (ChannelServer ce : cservs) {
                            ce.broadcastPacket(MaplePacketCreator.serverNotice(6, "[Hot Time Event] has ended in channel " + ch + "! Check our forums for the next hot time events!"));
                            ce.setServerMessage(ce.oldServerMessage);
                        }
                        che.hottimeTimer.cancel(true);
                        che.hottimeTimer = null;
                    }
                }, 1000 * 60 * 30);
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{
            ...
            new CommandDefinition("rmap", 4),
            new CommandDefinition("rdrops", 4),
            new CommandDefinition("fullmap", 4),
            new CommandDefinition("muteall", 4),
            new CommandDefinition("hottime", 4),
            new CommandDefinition("maintenance", 4),
            new CommandDefinition("mesos", 4)
        };
    }
}