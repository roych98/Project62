        ...
        if (mode == 4) { // Take out.
            ...
            if (item != null) {
                ...
                if (ItemConstants.trackedItems(item.getItemId())) {
                  FilePrinter.printError(FilePrinter.TrackedItems + 
                          c.getPlayer().getName() + ".txt", c.getPlayer().getName()+" just withdrew " + item.getQuantity() + " of "+ item.getItemId() + 
                                  ". Player was on Map " + c.getPlayer().getMapId() + ".\n");
                }
            } else {
            ...
            if (ItemConstants.trackedItems(itemId)) {
                  FilePrinter.printError(FilePrinter.TrackedItems + 
                          c.getPlayer().getName() + ".txt", c.getPlayer().getName()+" just stored " + quantity + " of "+ itemId +
                                  ". Player was on Map " + c.getPlayer().getMapId() + ".\n");
                }
            if (storage.isFull()) {
            ...
            if (c.getPlayer().getInventory(type).getItem(slot) == null) {
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
                }
