...
public class RangedHandler extends AbstractDealDamageHandler {

    private static org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RangedHandler.class);

    @Override
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        c.getPlayer().resetAfkTime();
        AttackInfo attack = parseDamage(slea, true, c.getPlayer());
        ...
        if (attack.skill == 5121002) {
            ...
            if (type == MapleWeaponType.NOT_A_WEAPON) {
                c.disconnect();
                //throw new RuntimeException("[Hacks] Player " + player.getName() + " is attacking with something that's not a weapon");
            }
