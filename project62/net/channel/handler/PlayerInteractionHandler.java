    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) { // Trade
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    if(c.getPlayer().hasMerchant()) {
                        c.getPlayer().dropMessage(1, "You already have a merchant. Nice PE.");
                        return;
                    }
                    String desc = slea.readMapleAsciiString();
                    slea.skip(3);
                    ...
                    if (c.getPlayer().haveItem(itemId, 1, false, true)) {
                        ...
                        AutobanManager.getInstance().autoban(c, " Attempts to open a shop without the item.");
                        return;
                    }
                } else {
            ...
            if (c.getPlayer().getTrade() != null && item != null) {
                if (checkq > 4000) {
                    AutobanManager.getInstance().autoban(c, "[AUTOBAN]PE while in trade.");
                }
            ...
            if (type == type.CASH) {
                System.out.println("Can't add cash items into a merchant... ");
                return;
            }
            ...
            System.out.println("perBundle: " + perBundle);*/
            ...
            sellItem.setQuantity(perBundle); // issue could be here
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                if (ivItem.getQuantity() >= bundles * perBundle) {
                    if (price <= 0) {
                        AutobanManager.getInstance().autoban(c, "[AUTOBAN]PE adding a negative priced item.");
                        return;
                    }
                    if (bundles <= 0 || perBundle <= 0 || checkquantity > 20000 || checksmquantity < 0 || checkiquantity < 0 || checkiquantity > 20000) {
                        AutobanManager.getInstance().autoban(c, "[AUTOBAN]PE Player Shop item: " + sellItem.getItemId());
                        return;
                    }
                    MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            ...
            IPlayerInteractionManager shop = c.getPlayer().getInteraction();
            if (shop != null && shop.isOwner(c.getPlayer()) && c.getPlayer().canHold(1000000)
                    && c.getPlayer().canHold(2000000)
                    && c.getPlayer().canHold(3000000)
                    && c.getPlayer().canHold(4000000)
                    && c.getPlayer().canHold(5000000)) {
                MaplePlayerShopItem item = shop.getItems().get(slot);
                if (item.getBundles() > 0) {
                    IItem iitem = item.getItem();
                    iitem.setQuantity((short) (item.getBundles() * item.getItem().getQuantity()));
                    MapleInventoryManipulator.addFromDrop(c, iitem);
                }
                shop.removeFromSlot(slot);
                c.getSession().write(MaplePacketCreator.shopItemUpdate(shop));
            } else {
                c.getPlayer().dropMessage("Clear some slots out!");
            }
        }  else if (mode == Action.CLOSE_MERCHANT.getCode()) {
            IPlayerInteractionManager merchant = c.getPlayer().getInteraction();
            if (merchant != null 
                    && merchant.getShopType() == 1 
                    && merchant.isOwner(c.getPlayer())) {
                boolean save = false;
                for (MaplePlayerShopItem items : merchant.getItems()) {
                    if (items.getBundles() > 0 && c.getPlayer().canHold(1000000)
                    && c.getPlayer().canHold(2000000)
                    && c.getPlayer().canHold(3000000)
                    && c.getPlayer().canHold(4000000)
                    && c.getPlayer().canHold(5000000)) {
