...
public class SkillBookHandler extends AbstractMaplePacketHandler {	
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (toUse != null && toUse.getQuantity() == 1) {
            ...
            if (player.getSkillLevel(SkillFactory.getSkill(skilldata.get("skillid"))) >= skilldata.get("masterLevel")) {//if skill level is greater than or equal to new mastery book skill level dont use it
                canuse = false;
            } else if (skilldata.get("skillid") == 0) { // Wrong Job.
                canuse = false;
