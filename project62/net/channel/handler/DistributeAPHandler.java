...
public class DistributeAPHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if(!c.getPlayer().isAlive()) {
            ...
            c.getPlayer().dropMessage(0, "You can not distribute abillity points when you're dead.");
            return;
        }
                ...
                case 2048: // HP
                    ...
                    if (c.getPlayer().getJob().isA(MapleJob.BEGINNER)) {
                        ...
                        if (improvingMaxHPLevel >= 1) {
                            MaxHP += rand(20, 25) + improvingMaxHP.getEffect(improvingMaxHPLevel).getY();
                        } else {
                            MaxHP += rand(20, 25);
                        }
                    } else if (c.getPlayer().getJob().isA(MapleJob.MAGICIAN)) {
                        MaxHP += rand(6, 12);
                    } else if ((c.getPlayer().getJob().isA(MapleJob.BOWMAN)) || c.getPlayer().getJob().isA(MapleJob.THIEF)) {
                        MaxHP += rand(16, 20);
