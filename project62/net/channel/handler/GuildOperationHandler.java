        ...
        switch (type) {
            ...
            case 0x06:
                ...
                try {
                    ...
                    c.getChannelServer().getWorldInterface().setGuildMemberOnline(mc.getMGC(), true, mc.getMGC().getChannel());//test
                ...
                mc.saveGuildStatus();
                respawnPlayer(mc);
                WorldChannelInterface wci = mc.getClient().getChannelServer().getWorldInterface();
                //test
                if(mc.getGuild() != null) {
                    if(mc.getGuild().getAllianceId() > 0) {
                        try {
                            wci.allianceMessage(mc.getGuild().getAllianceId(), MaplePacketCreator.updateAllianceJobLevel(mc), -1, -1);
                        }  catch (java.rmi.RemoteException e) {
                            e.printStackTrace();
                        }
                    }
