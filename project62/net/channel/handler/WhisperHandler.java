        ...
        if (channel == -1) {
            ...
            if (mode == 6) { // Whisper
                ...
                if (!(c.getPlayer().isGM()) && text.length() > 100) { // 70 = max text for client. remove this if u have edit the client
                    AutobanManager.getInstance().autoban(c.getPlayer().getClient(), "[Whisper Hack] " + c.getPlayer().getName() + " had infinite text with a text length of " + text.length() + ".");
                } else {
                ...
                if (c.getPlayer().getWatcher() != null) {
                 ...
                 FilePrinter.printError(FilePrinter.SPYCHAT + "" + ".txt","[" + c.getPlayer().getName() + " to " + recipient + " Whisper] : " + text + "\n");
                }  
            } else if (mode == 5) { // Find
                ChannelServer pserv = ChannelServer.getInstance(channel);
                MapleCharacter victim = pserv.getPlayerStorage().getCharacterByName(recipient);
