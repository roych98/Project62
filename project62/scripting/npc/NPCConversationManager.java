...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void forceStartQuest() {
        MapleQuest.getInstance(questid).forceStart(getPlayer(), getNpc());
    }
    ...
    public void gachNotice(MapleClient c, int itemid) {
        IItem itemToSmega;
        String msg = c.getPlayer().getName() + " has obtained ";
        if (itemid/1000000 == 1) {
            itemToSmega = c.getPlayer().getInventory(MapleInventoryType.EQUIP).findById(itemid);
            getPlayer().sendGachNotice(msg, c, itemToSmega);
        }
        else if (itemid/1000000 == 2) {
            itemToSmega = c.getPlayer().getInventory(MapleInventoryType.USE).findById(itemid);
            getPlayer().sendGachNotice(msg, c, itemToSmega);
        }
        else if (itemid/1000000 == 3) {
            itemToSmega = c.getPlayer().getInventory(MapleInventoryType.SETUP).findById(itemid);
            getPlayer().sendGachNotice(msg, c, itemToSmega);
        }
        else if (itemid/1000000 == 4) {
            itemToSmega = c.getPlayer().getInventory(MapleInventoryType.ETC).findById(itemid);
            getPlayer().sendGachNotice(msg, c, itemToSmega);
        }
    }
